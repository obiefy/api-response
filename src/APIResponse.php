...
class APIResponse
{
  ...
  public function response($status, $message, $data, ...$extraData)
  {
    ...
    if (is_countable($data) && config('api.includeDataCount', false) && !empty($data)) {
      $json = array_merge($json, [$this->dataCountLabel => config('api.stringify') ? strval(count($data)) : count($data)]);
    }
